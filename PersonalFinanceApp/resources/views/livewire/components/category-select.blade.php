...
{{--            </div>--}}
        ...
        <div ><h2 class="text-base font-bold {{$selected_category ? $this->is_color_dark($selected_category->category_color) ? 'text-black' : 'text-white' : ''}}">{{$selected_category ? $selected_category->category_name : 'Category'}}</h2></div>
        <div x-on:click="open = !open"><i class="fa-solid fa-plus" x-show="!open"></i><i class="fa-solid fa-minus" x-show="open"></i></div>
    </div>
    <div class="flex flex-wrap justify-center" x-show="open"  x-transition.opacity.duration.300ms>
        @foreach($categories as $category)
            {{--        <p>{{$category->category_name}}</p>--}}
            <div class="rounded-lg"
                 style="background-color: {{$this->selected_category == $category ? $category->category_color : '' }}">
                <input id="{{$transaction->id}}.{{$category->id}}.category-select" type="radio" class="hidden" wire:model="selected_category"
                       value="{{$category->id}}">
